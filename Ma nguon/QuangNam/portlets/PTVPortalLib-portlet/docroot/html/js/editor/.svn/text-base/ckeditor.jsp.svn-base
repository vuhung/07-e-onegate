...
String height = ParamUtil.getString(request, "height", "265");
%>
...
//			CKEDITOR.config.customConfig = '<%= request.getContextPath() %>/html/js/editor/ckeditor/ckconfig.jsp?p_l_id=<%= plid %>&p_main_path=<%= HttpUtil.encodeURL(mainPath) %>&doAsUserId=<%= HttpUtil.encodeURL(doAsUserId) %>&cssPath=<%= HttpUtil.encodeURL(cssPath) %>&cssClasses=<%= HttpUtil.encodeURL(cssClasses) %>&height=<%= height %>&toolbarSet=<%= TextFormatter.format(HtmlUtil.escape(toolbarSet), TextFormatter.M) %>';
...
<script type="text/javascript">
	...
	CKEDITOR.replace(
		'CKEditor1',
		{
			...
			customConfig: '<%= request.getContextPath() %>/html/js/editor/ckeditor/ckconfig.jsp?p_l_id=<%= plid %>&p_main_path=<%= HttpUtil.encodeURL(mainPath) %>&doAsUserId=<%= HttpUtil.encodeURL(doAsUserId) %>&cssPath=<%= HttpUtil.encodeURL(cssPath) %>&cssClasses=<%= HttpUtil.encodeURL(cssClasses) %>&height=<%= height %>&toolbarSet=<%= TextFormatter.format(HtmlUtil.escape(toolbarSet), TextFormatter.M) %>'
		}
	);
