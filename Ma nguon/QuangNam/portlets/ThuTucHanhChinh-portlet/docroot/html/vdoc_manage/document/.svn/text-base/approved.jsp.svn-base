...
function <portlet:namespace />doSearchDocuments() {
	...
	'<portlet:renderURL>
		...
		<portlet:param name="jspPage" value="/html/vdoc_manage/view.jsp?tabs1=document-tab&tabs2=portlet.vdoc.information-tab.approved"/>
	</portlet:renderURL>');
}
</script>
...
<%
	...
	renderTabApprovedURL.setParameter("jspPage", "/html/vdoc_manage/view.jsp?tabs1=document-tab&tabs2=portlet.vdoc.information-tab.approved");
%>
<br/><br/>
	...
	<liferay-ui:search-container-results>
		<%		
			...
			if(Validator.isNotNull(searchFlag) && Validator.equals(searchFlag, "true")){
				...
				vdocDocumentList = vdocDocumentLocalServiceUtil.getDocByG_L_S(groupId, language, vdocDocumentLocalServiceImpl.PUBLISH_PENDING);
			}
...
<aui:script >
	Liferay.provide(
		...
		if(document.<portlet:namespace />fm.<portlet:namespace />allRowIds.value.length>0)
		{
			...
			alert("You must select document before publish.");
			}
		},
		['liferay-util-list-fields']
	);
