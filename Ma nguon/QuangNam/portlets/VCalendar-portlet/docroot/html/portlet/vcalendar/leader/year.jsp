<script type="text/javascript">
	function <portlet:namespace />updateCalendar(month, day, year) {
		self.location = '<portlet:renderURL windowState="<%= WindowState.MAXIMIZED.toString() %>"><portlet:param name="tabs1" value="day" /><portlet:param name="status" value="<%= String.valueOf(status) %>" /><portlet:param name="tabs" value="<%= tabs %>" /></portlet:renderURL>&<portlet:namespace />month=' + month + '&<portlet:namespace />day=' + day + '&<portlet:namespace />year=' + year;
	}
</script>
...
<tr>
	<td>
		...
		<tr>
			<td>
				<a href="<%= leaderURL.toString() %>&<portlet:namespace />month=<%= selMonth %>&<portlet:namespace />day=<%= selDay %>&<portlet:namespace />year=<%= selYear - 1 %>&<portlet:namespace />status=<%= status %>&<portlet:namespace />tabs1=<%= "year" %>">
				<img border="0" src="<%= themeDisplay.getPathThemeImage() %>/arrows/01_left.gif">
				</a>
			</td>
			<td style="padding-left: 10px;"></td>
			<td>
				<select onChange="self.location = '<%= leaderURL.toString() %>&<portlet:namespace />tabs1=<%= "year" %>&<portlet:namespace />year=' + this.value;">
