<script type="text/javascript">
	function categoryTabView(categoryId) {						   
		   var url = '<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>">
			   				<portlet:param name="struts_action" value="/vcmsviewcontent/ajax"/>		   					
			   		  </portlet:renderURL>';			
			url = url + "&categoryId=" + categoryId;			
		   	jQuery('#ajax_table').load(url);
	    	...
	    	<td colspan="2">
				<table width="100%" align="left" class="tab_top_navi">
					<tr>
						...
						catURL.setParameter("struts_action", "/vcmsviewcontent/ajax");
						catURL.setParameter("sumAticles", String.valueOf(sumAticles));
						...
						%>				
							...
							<td id="select_center_top_tab"><div><b><a onClick="categoryTabView(<%=category.getCategoryId() %>)" ><%= category.getName() %></a></b></div></td>
							<td id="select_right_top_tab"></td>
						<%
							}else{
						%>
							<td id="normal_left_top_tab"><b><a onClick="categoryTabView(<%=category.getCategoryId() %>)" ><%= category.getName() %></a></b></td>
							<td id="normal_right_top_tab"></td>   
						<%
