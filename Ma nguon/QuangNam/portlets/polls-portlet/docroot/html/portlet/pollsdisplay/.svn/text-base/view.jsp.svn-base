...
<%@page import="com.vportal.portlet.pollsadmin.service.PollsQuestionServiceUtil"%>
...
<%@ include file="/html/portlet/pollsdisplay/init.jsp" %>
...
<%
PortletURL abc = renderResponse.createActionURL();
abc.setParameter(actionRequest.ACTION_NAME, "vote");
...
<form action="<%=abc.toString() %>" method="post" name="<portlet:namespace />fm">
					...
					<tr>
						...
						<td style="padding-top:10px;padding-bottom:5px;"  align="center"> 
							<input type="button"  value="<liferay-ui:message key="vote-results" />" onClick="var slideShowWindow = window.open('<portlet:renderURL windowState="<%= LiferayWindowState.POP_UP.toString() %>">
								<portlet:param name="struts_action" value="/vpolls/results" />
			...
			<tr>
				...
				<tr>
					<td style="padding-top:10px;padding-bottom:5px;" align="center">
						<input type="button" onClick="<portlet:namespace/>SubmitVote();" value="<liferay-ui:message key="vote" />"></td>									
					<td style="padding-top:10px;padding-bottom:5px;"  align="center"> 
						<input type="button"  value="<liferay-ui:message key="vote-results" />" onClick="var slideShowWindow = window.open('<portlet:actionURL name="showresults"  windowState="<%= LiferayWindowState.POP_UP.toString() %>">
							<portlet:param name="questionId" value="<%= String.valueOf(question.getQuestionId())%>" />
							</portlet:actionURL>', 'slideShow', 'directories=no,location=no,menubar=no,resizable=yes,width=400,height=<%= windowHeight%>,left='.concat((screen.width - 400)/2).concat(',top=').concat((screen.height - 300)/2)); void(''); slideShowWindow.focus();"> 
					</td>											
				</tr>
				</table>
				</td>	
